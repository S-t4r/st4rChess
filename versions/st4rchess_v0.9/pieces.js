export class Piece {
    ...
    Move(piece, newPosition) {
        ...
        if (this instanceof Pawn && this.position.y === 0 || this.position.y === 7)  {
            let piece;
    ...
    async updateLegalMoves(flag = false, castling = false) {
        // We need a base case.
        ...
        moveResults.forEach((result, index) => {
            if (result) {
                ...
                // Castling
                if (this instanceof King && !this.hasMoved && flag) {
                    if (!kingIsInCheck) {
                        this.castlingMoves();
                    }
                }
        ...
        let rooks = this.board.pieces.filter(piece => piece.color === this.color &&
            ...
            for (let rook of rooks) {
                ...
                i < Math.max(this.position.x, rook.position.x); i++) {
                    ...
                    moveArray.push({x: i, y: this.position.y})
                    castling = true;
                }
                ...
                if (castling) {
                    ...
                    // Wait to see if any of the squares in between are under attack.
                    let underAttack = async () => {
                        
                        // Store value in a variable
                        let checkBetween = await this.board.isSquareUnderAttack(this.color, moveArray);

                        // If rook is on the king's left.
                        // Queen side castling
                        if (this.position.x > rook.position.x && !checkBetween) {
