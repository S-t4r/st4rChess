export class Piece {
    ...
    Move(piece, newPosition) {
        ...
        // Check for captures
        if (newPosition instanceof Piece && newPosition.color !== piece.color) {
            piece.Capture(newPosition);
        }
    ...
    Capture(designation) {
        ...
        this.board.capturedPieces.push(designation);
        if (designation.square != null) {
            designation.square.css({backgroundColor:
                designation.square.data('originalColor')});
                }
            designation.exists = false;
            designation.square = null;
...
export class Pawn extends Piece {
    ...
    Draw(where) {
        ...
        // If the piece already exists
            ...
            this.square.on('click', () => {
                ...
                else if (!this.clicked && this.square != null) {
                    // Select the piece
    ...
    PawnMove(newPosition) {
        if (this.PawnCapture(newPosition)) {
            return true;
        }
        // Check in front of
    ...
    PawnCapture(newPosition) {
        // Check diagonally in front
        if (Math.abs(newPosition.y - this.position.y) === 1 &&
        Math.abs(newPosition.x - this.position.x) === 1) {
            let destinationPiece = this.board.board[newPosition.x][newPosition.y];
            if (destinationPiece != null && destinationPiece.color !== this.color) {
                this.Capture(destinationPiece);
                return true;
            }
        }
        else {
